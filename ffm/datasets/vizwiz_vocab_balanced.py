...
def main():
    ann_file = "./vizwiz/ann.json"   # now reading from gqa/ann.json
    out_dir  = "vizwiz"
    ...
    all_answers = [
        rec["answers"][0]['answer'].strip().lower()
        for rec in data
        if isinstance(rec.get("answers")[0]['answer'], str) and rec["answers"][0]['answer'].strip()
    ]
